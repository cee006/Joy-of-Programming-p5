...
function createBox() {
  // prepare a box first
  strokeWeight(4);
  rect(100, 100, BOX_WIDTH, BOX_HEIGHT);
  ...
  textAlign(CENTER);
  text('Hello World!',170, BOX_WIDTH/2, BOX_HEIGHT/2);   
...
class Ball { // Constructor
  ...
  checkForHitWall() {
    ...
    if ((this.ballY+radius) > height || (this.ballY-radius) < 0) {
      ...
      this.green = random(255);
      this.blue  = random(255);
      this.display(this.red, this.green, this.blue, 100);
  	}
...
//    if (((this.ballX-radius) < BOX_WIDTH && (this.ballY-radius) < BOX_HEIGHT) || d < radius) {
  ...
  reverseBall() {
      ...
      this.speedX = -this.speedX;
      this.speedY = -this.speedY;
      // changes the color of the ball when it reverses directions 
      this.red   = random(255);
      this.green = random(255);
      this.blue  = random(255);
      this.display(this.red, this.green, this.blue, 100);
